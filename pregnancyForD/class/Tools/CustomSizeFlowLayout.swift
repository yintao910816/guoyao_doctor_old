...
@objc public protocol CustomSizeDelegateFlowLayout: UICollectionViewDelegate, UICollectionViewDataSource {
...
open class CustomSizeFlowLayout : UICollectionViewLayout{
    ...
    var firstSectionItems = [[Int]]() {
        didSet {
            var count = 0
            for list in firstSectionItems {
                count += list.count
            }
            self.firstSectionItemsCount = count
            self.invalidateLayout() // 更新布局
        }
    }
    
    var firstSectionItemsCount = 0
    
    fileprivate var lineSpacing: CGFloat = 0.0
    
    fileprivate var interitemSpacing: CGFloat = 0.0
    
    fileprivate weak var delegateFlowLayout : CustomSizeDelegateFlowLayout?{
        get{
            return self.collectionView?.delegate as? CustomSizeDelegateFlowLayout
        }
    }
    /// 显示所需高度
...
//    }
    ...
    fileprivate func prepareLayoutGuard() -> Bool {
        ...
        guard self.delegateFlowLayout != nil else {
            print("error: CustomSizeDelegateFlowLayout未实现")
            return false
        }
    ...
    fileprivate func prepareLayoutSectionCell(_ section: Int) {
        ...
        if firstSectionItemsCount % 2 == 0 {
            numberOfSectionItems = (numberOfItemsInSection - 1) / self.sectionItemsCount + 1
        }else{
            // 自定义  计算有多少行
            numberOfSectionItems = (numberOfItemsInSection - 2) / self.sectionItemsCount + 1
        }
        ...
        for numberOfSectionItem in 0..<numberOfSectionItems {
            ...
            if numberOfSectionItem == 0 {
                for (indexSectionItem, sectionItem) in self.firstSectionItems.enumerated() {
                    //                sectionItems = [[0], [1,5], [2,6],[3,7],[4,8]]
                ...
                for (indexSectionItem, sectionItem) in self.sectionItems.enumerated() {
                    ...
                    for var item in sectionItem {
                        if firstSectionItemsCount % 2 == 0 {
                            ...
                            item = self.sectionItemsCount * numberOfSectionItem + item + 1
                        }
                        guard item < numberOfItemsInSection else { // 安全处理
                            break
                        }
